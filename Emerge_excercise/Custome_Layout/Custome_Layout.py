...
def show_layout(layout: LayoutData) -> None:
    """생성된 픽셀 패턴을 간단한 2D 창으로 표시합니다."""
    try:
        import matplotlib.pyplot as plt
        from matplotlib.patches import Rectangle
    except ModuleNotFoundError as exc:
        raise SystemExit(
            "레이아웃 뷰어에는 matplotlib가 필요합니다. "
            "'python -m pip install matplotlib'로 설치해주세요."
        ) from exc
    ...
    ax.add_patch(Rectangle(
        (0, 0), cfg.board_size, cfg.board_size,
        facecolor="#4d7c0f", edgecolor="black", alpha=0.35,
    ))
    ...
    for px in layout.pixels:
        ax.add_patch(Rectangle(
            ...
            facecolor="#ffc107", edgecolor="none",
        ))

    # 양쪽 50 Ω 포트 연결부
    port_y = cfg.port_y - cfg.port_width / 2
    ax.add_patch(Rectangle(
        (cfg.bd_left - cfg.port_length, port_y),
        cfg.port_length, cfg.port_width, facecolor="#ffc107", edgecolor="none",
    ))
    ax.add_patch(Rectangle(
        (cfg.bd_right, port_y),
        cfg.port_length, cfg.port_width, facecolor="#ffc107", edgecolor="none",
    ...
    ax.set_xlim(0, cfg.board_size)
    ax.set_ylim(0, cfg.board_size)
    ax.set_aspect("equal")
    ax.set_xlabel("X [mm]")
    ax.set_ylabel("Y [mm]")
    ax.set_title(f"Custom PCB Layout ({len(layout.pixels)} pixels)")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Custom pixel layout viewer")
    parser.add_argument("--no-gui", action="store_true", help="레이아웃 창을 열지 않음")
