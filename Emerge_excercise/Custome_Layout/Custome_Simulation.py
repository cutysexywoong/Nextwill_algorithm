...
def main(output_dir: Path = None, visualize: bool = False):
    if output_dir is None:
        output_dir = Path(__file__).resolve().parent / "Coral2"
    ...
    print("Custome Custom Layout V3 Generated")
    ...
    # 4~10GHz 해석: 공기 및 기판 메쉬는 최고 주파수 10GHz를 기준으로 계산
    MAX_FREQ = 10e9
    # 미세 형상 메쉬는 이전의 14GHz 기준값을 그대로 유지
    FINE_MESH_REFERENCE_FREQ = 14e9

    c = 3e8
    lambda_air = (c / MAX_FREQ) * 1000
    lambda_val = (c / MAX_FREQ) * 1000 / np.sqrt(er)
    fine_lambda_val = (c / FINE_MESH_REFERENCE_FREQ) * 1000 / np.sqrt(er)

    # 공기와 기판 매질만 10GHz 기준으로 변경하고 작은 형상은 기존 크기 유지
    MAX_EDGE = lambda_air
    MIN_EDGE = fine_lambda_val / 4
    PCB_EDGE_SIZE = lambda_val / 2
    Hair = 9.375
    ...
    layouter.add_poly(
        [port1_x_start, config.bd_left, config.bd_left, port1_x_start],
        [port_y_min, port_y_min, port_y_max, port_y_max],
    )

    port2_x_end = config.bd_right + PORT_L
    layouter.add_poly(
        [config.bd_right, port2_x_end, port2_x_end, config.bd_right],
        [port_y_min, port_y_min, port_y_max, port_y_max],
    )
    ...
    # 4~10GHz, 0.5GHz 간격: 총 13개 포인트
    freq_list = [4e9 + index * 0.5e9 for index in range(13)]

    m.mw.set_frequency(freq_list)
    print(f"📊 Frequency points: {len(freq_list)}")
    print("📊 Frequencies [GHz]:", [f / 1e9 for f in freq_list])
