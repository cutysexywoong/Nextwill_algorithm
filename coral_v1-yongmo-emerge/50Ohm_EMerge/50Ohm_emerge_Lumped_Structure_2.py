...
m = em.Simulation('50ohm_line_pyvista_2')
...
layouter.new(X_OFFSET + START_X_OFFSET, CENTER_Y, PORT_W, (1,0)).store('p1')\
    .straight(LINE_LENGTH, PORT_W).store('p2')

polies = layouter.compile_paths(merge=True)

y_common_margin = (TOTAL_HEIGHT - PORT_W) / 2
...
print("✅ PyVista 시각화 종료 (메쉬 없음, Lumped Port 포함)")
